-- ZZZZ: 10.5pt -- well done! -- see comments below
...
-- ZZZ: -0.5pt; our solution below:

lemma monotone_of_continuous_ZZZ {α : Type} (f : set α → set α)
    (hf : continuous f) :
  monotone f :=
begin
  intros A B ha,
...
lemma lfp_Kleene_ZZZ {α : Type} (f : set α → set α) (hf : continuous f) :
  lfp f = Union (λi, (f ^^ i) ∅) :=
begin
  apply le_antisymm,
  { apply lfp_le _ _ _,
